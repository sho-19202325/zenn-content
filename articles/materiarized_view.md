...
この結果セットは元テーブルのデータとは独立しているため、もし元のテーブル(上記だとusers)に新たなレコードが追加されたとしても、<br/>
...
真っ先に思いつくのが「元のテーブルのデータが更新されるたびに更新する」だと思います。<br/>
...
::message
再作成ではなく、差分だけを検知してビューの更新を行うための[pg_ivm](https://github.com/sraoss/pg_ivm)というプラグインが存在するようです。<br/>
このプラグインの開発者の方が、PostgreSQLの開発コミュニティに機能提案をしているそうで、もしかすると今後PostgreSQL単体で差分だけを検知して、<br/>
リフレッシュする機能が追加されることになるかもしれません。
詳しく知りたい方は下記をご参照ください。
